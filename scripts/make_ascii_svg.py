...
portrait prints once and freezes.
    ...
    python scripts/make_ascii_svg.py [source-prepped.png] [vetle-ascii.svg]
...
OUT = sys.argv[2] if len(sys.argv) > 2 else os.path.join(HERE, "..", "vetle-ascii.svg")
...
# Tuned for this photo. The article's defaults (1.05 / 1.18 / 0.80) were
# calibrated for a tight head crop; they blank out the skin here and leave a
# hollow face. Higher contrast + a higher white floor keeps facial tone while
# still washing the background to blank.
CONTRAST = 1.25
BRIGHTNESS = 1.0
...
# ui-monospace stack. Used to park the status-bar cursor flush with the text
...
status_text = f"{HOST}:~$ whoami {NAME}"
status_cursor_x = PAD + len(status_text) * 13 * MONO_ADVANCE + 4
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{HOST}:~$ whoami <tspan fill="{INK}">{NAME}</tspan></text>')
parts.append(f'<rect x="{status_cursor_x:.1f}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
